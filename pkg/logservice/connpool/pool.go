...
var (
	// ErrClosedMsg is the error message indicates the connection pool is closed.
	ErrClosedMsg = "connection pool is closed"
	// ErrTimeoutMsg is the error message indicates the waiting timed out.
	ErrTimeoutMsg = "connection pool timeout"
...
func (p *ConnPool) wait(ctx context.Context) (bool, error) {
	...
	case p.cond <- struct{}{}: // the pool has connection
		return true, nil
	default:
	}
	...
	case <-timer.C:
		...
		return false, moerr.NewInternalError(p.ctx, ErrTimeoutMsg)
	}
}
...
func (p *ConnPool) push(conn *Conn) error {
	...
	if p.closed {
		_ = conn.Close()
		return moerr.NewInternalError(p.ctx, ErrClosedMsg)
	}
...
func (p *ConnPool) pop() (*Conn, error) {
	...
	defer p.mu.Unlock()
	if p.closed {
		return nil, moerr.NewInternalError(p.ctx, ErrClosedMsg)
	}
	l := len(p.pool)
	if l == 0 {
		return nil, nil
